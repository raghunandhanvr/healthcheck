...
export class SessionRepository extends BaseRepository<Session> {
  ...
  async findActiveByUserId(userId: string): Promise<Session[]> {
    return this.rawQuery('SELECT * FROM "session" WHERE "userId" = $1 AND "expiresAt" > NOW()', [
      userId,
    ])
  }

  async findExpiredSessions(): Promise<Session[]> {
    return this.rawQuery('SELECT * FROM "session" WHERE "expiresAt" <= NOW()')
  }
  ...
  async deleteExpiredSessions(): Promise<Session[]> {
    return this.rawQuery('DELETE FROM "session" WHERE "expiresAt" <= NOW() RETURNING *')
  }
